...
func TestReconcile(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			client:        fake.NewClientBuilder().WithObjects(edsNode1).Build(),
			want:          reconcile.Result{},
			wantStatusErr: false,
		},
		{
			name:          "With a node",
			client:        fake.NewClientBuilder().WithObjects(node1, edsNode1).Build(),
			want:          reconcile.Result{},
			wantStatusErr: false,
		},
		{
			name:          "Nil reference",
			client:        fake.NewClientBuilder().WithObjects(node1, edsNode3).Build(),
			want:          reconcile.Result{},
			wantStatusErr: true,
		},
		{
			name:          "Empty reference name",
			client:        fake.NewClientBuilder().WithObjects(node1, edsNode4).Build(),
			want:          reconcile.Result{},
			wantStatusErr: true,
		},
		{
			name:          "Conflict",
			client:        fake.NewClientBuilder().WithObjects(node1, edsNode1, edsNode2).Build(),
			want:          reconcile.Result{},
			wantStatusErr: true,
		},
	}
